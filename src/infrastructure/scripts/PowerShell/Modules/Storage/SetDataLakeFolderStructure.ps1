[CmdletBinding()]
    ...
    do {
        $newItems = Get-AzDataLakeGen2ChildItem -FileSystem $ContainerName -Recurse -MaxCount $MaxReturn -ContinuationToken $Token -Context $ctx
        if($null -eq $newItems) {
            $Token = $null
        } else {
            $items.AddRange($newItems)
            $Token = $newItems[$newItems.Count -1].ContinuationToken;
        }
    } while ($null -ne $Token)

    Write-Host "Read $($items.Count) items from DataLake"
